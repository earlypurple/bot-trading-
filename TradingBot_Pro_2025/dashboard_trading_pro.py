...
-----END EC PRIVATE KEY-----"""
    ...
    def tendance_strategy(self, df):
        ...
        try:
            ...
            current_sma_10 = df['sma_10'].iloc[-1]
            current_sma_20 = df['sma_20'].iloc[-1]
            prev_sma_10 = df['sma_10'].iloc[-2]
            prev_sma_20 = df['sma_20'].iloc[-2]
            
            # Signal d'achat : SMA courte croise au-dessus SMA longue
            if prev_sma_10 <= prev_sma_20 and current_sma_10 > current_sma_20:
                strength = min(((current_sma_10 - current_sma_20) / current_sma_20) * 1000, 100)
                return 'BUY', strength
            
            # Signal de vente : SMA courte croise en-dessous SMA longue
            elif prev_sma_10 >= prev_sma_20 and current_sma_10 < current_sma_20:
                strength = min(((current_sma_20 - current_sma_10) / current_sma_10) * 1000, 100)
                return 'SELL', strength
            
            return 'HOLD', 0
            
        except Exception as e:
            print(f"❌ Erreur SMA Cross: {e}")
            return 'HOLD', 0
    ...
    def momentum_strategy(self, df):
        ...
        try:
            ...
            if current_rsi < 30:
                strength = (30 - current_rsi) * 3.33  # Max 100%
                return 'BUY', min(strength, 100)
            
            # RSI > 70 = Surachat (Vente)
            elif current_rsi > 70:
                strength = (current_rsi - 70) * 3.33  # Max 100%
    ...
    def convergence_strategy(self, df):
        """Stratégie MACD"""
        try:
            ...
            if prev_macd <= prev_signal and current_macd > current_signal:
                strength = min(abs(current_macd - current_signal) * 10000, 100)
                return 'BUY', strength
            
            # MACD croise en-dessous signal
            elif prev_macd >= prev_signal and current_macd < current_signal:
                strength = min(abs(current_signal - current_macd) * 10000, 100)
                return 'SELL', strength
    ...
    def volatilite_strategy(self, df):
        ...
        try:
            ...
            # Prix touche bande inférieure (Achat)
            if current_price <= current_lower:
                strength = ((current_lower - current_price) / current_lower) * 1000
                return 'BUY', min(strength, 100)
            
            # Prix touche bande supérieure (Vente)
            elif current_price >= current_upper:
                strength = ((current_price - current_upper) / current_upper) * 1000
                return 'SELL', min(strength, 100)
    ...
    def analyze_market(self, symbol='BTC/USD'):
        """Analyse de marché avec stratégies multiples"""
        try:
            # Récupérer les données OHLCV
            ohlcv = self.exchange.fetch_ohlcv(symbol, '5m', limit=100)
            df = pd.DataFrame(ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
            
            if len(df) < 50:  # Pas assez de données
                return None
    ...
    <script>
        ...
        function startBot() {
            ...
            .catch(error => {
                ...
                showNotification('Erreur lors du démarrage du bot', 'error');
            })
            .finally(() => {
                if (startButton) {
...
def start_bot_api():
    ...
    try:
        ...
        print(f"❌ Erreur démarrage bot API: {e}")
